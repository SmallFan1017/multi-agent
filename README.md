...
> **A multi-agent research workflow system for ASR graduate research.**
> Orchestrates 7 specialized AI sub-agents (translation, summarization, code review,
> literature search, presentation, data analysis, web verification) via a single main agent,
> accessible through both VS Code Copilot and Claude CLI.
> Built on prompt engineering principles to standardize and automate high-frequency research tasks.
...
翻譯與摘要一律優先讀取本地 PDF（`output/papers/`），避免網頁內容不完整造成資訊偏差。arXiv 網頁版有時缺圖、公式渲染不完整，實測發現從 HTML 抓取的翻譯品質明顯下降。
...
多工具切換下，格式不一致使輸出難以整合進 HackMD 筆記或報告；統一規範後可直接貼入使用。
...
長論文全文翻譯曾觸發 context limit，分批處理確保每篇都能完整產出。

### 4) 任務後自我修正（Post-Task Review）

在符合條件時，會回顧流程中的冗餘步驟並更新代理規範，以持續優化效率。初版 paper-summarizer 會先提取所有段落再整理，造成重複讀取；修正後改為邊讀邊整理，省去一輪冗餘步驟。
...
> **路徑說明**：文件中的 `C:\path\to\multi-agent` 請替換為你實際 clone 的專案路徑，`C:\Users\YourName\` 請替換為你的使用者目錄。
