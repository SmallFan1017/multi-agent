...
│   └── archive/                           ← 歸檔（不影響執行）
│       ├── agent-planning.md              ← 舊規劃文件
│       ├── prompt.txt                     ← 舊版 prompt
│       └── gcp-espnet-plan.md             ← 舊專案規劃
...
1. **開啟此工作區**：在 VS Code 中開啟 `Agent/` 資料夾
2. **開啟 Copilot Chat**：按 `Ctrl+Shift+I` 或點擊側邊欄的 Copilot 圖示
3. **切換到 Agent Mode**：在 Chat 面板頂部，切換模式為 **Agent**
4. **使用主 Agent**：直接輸入任務描述
   ```
   幫我翻譯這篇論文：C:\Users\user\Downloads\paper.pdf
   ```
5. **直接使用 Sub-Agent**：用 `@` 指定
   ```
   @paper-translator 翻譯這篇論文：C:\Users\user\Downloads\paper.pdf
   輸出到：C:\Users\user\Downloads\output\
   ```
...
1. **在終端機進入此工作區**：
   ```powershell
   cd C:\Users\user\workspace\Agent
   ```
...
```
幫我翻譯這篇論文，輸出中英對照格式
論文：C:\Users\user\Downloads\課程相關\some_paper.pdf
輸出到：C:\Users\user\Downloads\課程相關\翻譯\
```
...
幫我整理這篇論文的重點
論文：C:\Users\user\Downloads\課程相關\some_paper.pdf
輸出到：C:\Users\user\Downloads\課程相關\論文筆記\
```
...
幫我處理這篇論文：翻譯並整理重點
論文：C:\Users\user\Downloads\課程相關\some_paper.pdf
輸出到：C:\Users\user\Downloads\課程相關\output\
```
...
預設情況下，Agent 只在 `Agent/` 資料夾內有效。以下是在其他專案目錄使用的方法：

#### VS Code Copilot — 加入多根工作區（Multi-root Workspace）

最簡單的方式：將 `Agent/` 資料夾加入你目前的 VS Code 工作區。

1. 在 VS Code 選單：**檔案 → 將資料夾加入工作區**
2. 選擇 `C:\Users\user\workspace\Agent`
...
可以在 `C:\Users\user\` 建立一個轉接檔，讓所有子目錄都能使用此 Agent：

```markdown
<!-- C:\Users\user\CLAUDE.md -->
# 全域研究生 Agent
請讀取並遵循以下檔案的指令：
C:\Users\user\workspace\Agent\CLAUDE.md

Sub-Agent Prompt 的絕對路徑：
- 論文翻譯：C:\Users\user\workspace\Agent\agents\paper-translator\AGENTS.md
- 論文整理：C:\Users\user\workspace\Agent\agents\paper-summarizer\AGENTS.md
- 程式碼審查：C:\Users\user\workspace\Agent\agents\code-reviewer\AGENTS.md
- 簡報製作：C:\Users\user\workspace\Agent\agents\presentation-maker\AGENTS.md
- 文獻搜尋：C:\Users\user\workspace\Agent\agents\literature-search\AGENTS.md
- 數據分析：C:\Users\user\workspace\Agent\agents\data-analysis\AGENTS.md
```

這樣在 `C:\Users\user\` 下的任何子目錄執行 `claude`，都能自動載入此 Agent。
...
A：可以，但需要設定。VS Code 請用「多根工作區」把 Agent/ 加進任何專案；Claude CLI 請在 `C:\Users\user\` 建立一個轉接 CLAUDE.md（詳見四、自訂與擴展）。
